...
const handleSave = async () => {
  try {
    ...
    const response = await axios.put(`http://192.168.68.228:3001/loan/${borrower._id}`, editData, {
      headers: {
  ...
  const fetchBorrower = async () => {
    try {
      const [borrowerResponse, cashflowResponse, comakerResponse] = await Promise.all([
        fetch(`http://192.168.68.228:3001/api/borrower/${loanId}`),
        fetch(`http://192.168.68.228:3001/api/cashflow/${loanId}`),
        fetch(`http://192.168.68.228:3001/api/comaker/${loanId}`)
      ]);
